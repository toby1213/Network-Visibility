...
class Trie(object):
	...
	def deleteFromTrie(self, ip, node = None, path = ""):
		if (node is None):
			node = self.root
		generatedEcs = []
		#recursively examine each digit in the ip. if 0, go left; if 1, go right; if *, go star
		currentChar = ip[0];	#recursively delete the rule from the trie, and return the set of affected ECs; if the rule is not in the trie, return an empty set
		if (currentChar == '*'):
			...
			ecLeft = self.deleteFromTrie(ip[1:], node.getLeftChild(), path + "0");	#first digit in the ip is 0
...
# newly added methods	
	def getECfromRule(self, rule_prefix=None, node=None, path=""):
		if(node is None):
			node = self.root
		if (rule_prefix == ""):
			return node.getEcs();
		currentChar = rule_prefix[0];
		if (currentChar == '0'):
			return self.getECfromRule(rule_prefix[1:], node.getLeftChild(), path + "0");
		else:
			return self.getECfromRule(rule_prefix[1:], node.getRightChild(), path + "1");

	def printTrie(self, node, prefix):
		if node is None:
			return
		print("prefix:", prefix, end=" ")
		# print(len(node.getEcs()))
		for ec in node.getEcs():
			print("ec:", ec.toString(), end=" ")
		print()
		self.printTrie(node.getLeftChild(), prefix + "0")
		self.printTrie(node.getRightChild(), prefix + "1")
		self.printTrie(node.getStarChild(), prefix + "*")
